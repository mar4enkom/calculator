import {CalculatorUIKit} from "../helpers/ui/CalculatorUIKit";
...
import {BasicOperationList, DigitSymbols, FunctionOperationList, Symbols} from "@calculator/common";

export class CalculatorViewService implements CalculatorViewServiceInterface {
    ui: CalculatorUIKit;
    private digitSymbols: DigitSymbols;
    constructor(symbols: Symbols, digitSymbols: DigitSymbols) {
        this.ui = new CalculatorUIKit(symbols);
        this.ui.inputElement.focus();
        this.digitSymbols = digitSymbols;
    }
    ...
    renderDigitButtonList(root: HTMLDivElement): void {
        const numberList = getDigitColumnItems(this.digitSymbols).map(number => ({sign: number}));
